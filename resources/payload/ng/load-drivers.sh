...
set -eu
fail() { echo "Aurora 6S NG: $*" >&2; exit 1; }
[ "$(uname -r)" = '4.9.269' ] &&
...
[ "$(tr -d '\000' < /proc/device-tree/coreelec-dt-id)" = sc2_s905x4_tencent_aurora_6s_ng ] || fail 'Install the Aurora 6S NG DTB and reboot first.'
